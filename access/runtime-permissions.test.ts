...
Deno.test("parsePermissionSpec: scoped net maps to `host`, env to `variable`, run to `command`, sys to `kind`", () => {
  assertEquals(parsePermissionSpec("net:api.example.com"), { host: "api.example.com", name: "net" });
  assertEquals(parsePermissionSpec("net:host:8080"), { host: "host:8080", name: "net" });
  assertEquals(parsePermissionSpec("env:DATABASE_URL"), { name: "env", variable: "DATABASE_URL" });
  assertEquals(parsePermissionSpec("run:git"), { command: "git", name: "run" });
  assertEquals(parsePermissionSpec("sys:hostname"), { kind: "hostname", name: "sys" });
});
...
Deno.test("parser composes runtime::has_permission with AND", () => {
  const expr = parseExpr(`runtime::has_permission("net") and current_user.is_admin`);
...
Deno.test("expressionToSQL: composes with existing globals (admin and granted permission)", () => {
  ...
  const expr = parseExpr(`current_user = "alice" and runtime::has_permission("net")`);

  const sql = ev.expressionToSQL(expr, {
    ...makeCtx(() => "granted"),
    userId: "alice"
  });
...
Deno.test("expressionToSQL: denied permission short-circuits to FALSE in composed expression", () => {
  ...
  const expr = parseExpr(`current_user = "alice" and runtime::has_permission("ffi")`);
  ...
  /*** The AND clause survives, but the runtime check is FALSE, so Postgres will evaluate the row
       filter as false and return zero rows. ***/
