...
import { type EmbeddedSdkEntry, extractEmbeddedSdkWithEntries } from "./sdk-extractor.ts";
 ...
 */
async function makeFakeSdkSources(
  tmp: string
): Promise<{ entries: EmbeddedSdkEntry[]; sourceDir: string; }> {
...
Deno.test("extractEmbeddedSdk - first run writes every entry + marker", async () => {
  ...
  try {
    ...
    const result = await extractEmbeddedSdkWithEntries(
      targetDir,
      "2026.05.07",
      entries
    );
...
Deno.test("extractEmbeddedSdk - second run with same version is a no-op", async () => {
  ...
  try {
    ...
    const first = await extractEmbeddedSdkWithEntries(
      targetDir,
      "2026.05.07",
      entries
    );
    ...
    // Mutate one of the extracted files to prove the no-op path doesn't
    // re-write — the user's manual changes survive a second codegen run
    // when the binary version hasn't changed.
    await Deno.writeTextFile(
      join(targetDir, "client.ts"),
      "// user edited\n"
    );

    const second = await extractEmbeddedSdkWithEntries(
      targetDir,
      "2026.05.07",
      entries
    );
...
Deno.test("extractEmbeddedSdk - marker with different version triggers re-extract", async () => {
  const tmp = await Deno.makeTempDir({
    prefix: "disc-sdk-extractor-version-bump-"
  });
  try {
    const { entries } = await makeFakeSdkSources(tmp);
    const targetDir = join(tmp, "out", "sdk");
    ...
    // User modifies a file; should be overwritten when the binary
    // version bumps because the SDK might have changed shape.
    await Deno.writeTextFile(
      join(targetDir, "client.ts"),
      "// stale user edit\n"
    );
