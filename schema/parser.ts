...
export class SDLParser {
  ...
  private parseEqualityExpression(): AST.Expression {
    ...
    while (true) {
      ...
      if (this.match(TokenType.EQUALS)) {
        ...
        op = "!=";
      } else if (this.match(TokenType.QUESTIONEQ)) {
        op = "?=";
      } else if (this.match(TokenType.QUESTIONNEQ)) {
        op = "?!=";
      }
  ...
  private parseUnaryExpression(): AST.Expression {
    ...
    // `<` at expression-start position is unambiguous — comparison only
    // appears between two expressions, never as a leading token.
    if (this.match(TokenType.LESS)) {
      const type = this.parseTypeRef();
