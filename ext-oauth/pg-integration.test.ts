...
Deno.test({
  ...
  fn: async () => {
    ...
    // disc_oauth_identities defaults id to disc_uuidv7(); bootstrap the stdlib
    // function first, exactly as DiscServer.start() does before extension setup.
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const result = await pool.query(
        `SELECT id::text, user_id::text, provider, provider_user_id, email, name
      ...
      assertEquals(result.rows.length, 1);
      // id is server-generated by the disc_uuidv7() default — verify v7.
      assertEquals(String(result.rows[0]["id"])[14], "7");
