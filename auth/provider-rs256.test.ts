...
describe("AuthProvider — RS256", () => {
  ...
  it("registers a user and mints an RS256 token", async () => {
    ...
    const response = await provider.register({ email: "rs@example.com", password: "SecurePass123!" });
  ...
  it("verifies its own RS256 tokens end-to-end", async () => {
    ...
    const response = await provider.register({ email: "verify@example.com", password: "SecurePass123!" });
  ...
  it("rejects HS256-signed tokens", async () => {
    /*** Mint an HS256 token from a parallel HS256 provider, then ask the RS256 provider to verify —
         must fail because the algorithm doesn’t match the imported key. Same DB instance is reused
         so the session row exists; only the signature check should fail. ***/
    const hsProvider = new AuthProvider({ bcryptRounds: 4, jwtSecret: "test-secret-key-for-testing-only-32+" }, db as any);
    await hsProvider.initialize();
    const hsResponse = await hsProvider.register({ email: "cross@example.com", password: "SecurePass123!" });
  ...
  it("the reverse: HS256 provider rejects RS256-signed tokens", async () => {
    ...
    await rsProvider.initialize();
    const rsResponse = await rsProvider.register({ email: "rev@example.com", password: "SecurePass123!" });
  ...
  it("throws on malformed PEM armor", async () => {
    ...
    await assertRejects(() => provider.initialize(), Error, "PEM key missing");
  });

  it("HS256 still requires jwtSecret (regression — config validation)", async () => {
    /*** No jwtSecret, no jwtAlgorithm → defaults to HS256 ***/
    const provider = new AuthProvider({ bcryptRounds: 4 } as AuthConfig, db as any);
    await assertRejects(() => provider.initialize(), Error, "jwtSecret");
  });

  it("dropping jwtAlgorithm preserves HS256 default behavior", async () => {
    const provider = new AuthProvider({ bcryptRounds: 4, jwtSecret: "test-secret-key-for-testing-only-32+" }, db as any);
    await provider.initialize();

    const response = await provider.register({ email: "default@example.com", password: "SecurePass123!" });
    const headerB64 = response.token.split(".")[0];
    const header = JSON.parse(atob(headerB64.replace(/-/g, "+").replace(/_/g, "/")));
...
async function generateRsaPemPair(): Promise<{ privatePem: string; publicPem: string; }> {
