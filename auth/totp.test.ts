...
/*** Spec uses ASCII secret "12345678901234567890" (= 20 bytes). Base32 of that
...
for (const v of RFC_VECTORS) {
  Deno.test(`RFC 6238 vector: SHA-1, t=${v.timestampSeconds}`, async () => {
    const code = await generateTOTP(RFC_SECRET_SHA1, { digits: 8, timestampMs: v.timestampSeconds * 1000 });
...
Deno.test("buildOtpauthUri — has issuer, account, secret, defaults to SHA1/6/30", () => {
  const uri = buildOtpauthUri({ accountName: "user@example.com", issuer: "Disc", secret: "JBSWY3DPEHPK3PXP" });
...
Deno.test("buildOtpauthUri — URL-encodes spaces in issuer", () => {
  const uri = buildOtpauthUri({ accountName: "alice", issuer: "My App", secret: "ABCDEFGH" });
