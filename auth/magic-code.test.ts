...
Deno.test("verifyMagicCode — when user has TOTP, returns MfaChallenge and burns the code", async () => {
  ...
  try {
    ...
    // Code must have been consumed even though we got a challenge —
...
Deno.test("generateNumericCode — leading-digit distribution is roughly uniform across 10K samples", async () => {
  const { provider, db } = await makeProvider();
  try {
    // Reach into the generator via a public seam: requesting codes for
    // a non-existent user still produces plaintext via the same path.
