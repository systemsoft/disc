...
export async function verifyAssertionSignature(opts: {
  ...
  // ES256 raw signatures from authenticators are DER-encoded ECDSA
  // (r,s sequences). WebCrypto wants raw r||s 64-byte form; convert.
...
// ── CBOR decoder ──────────────────────────────────────────────────────
//
// Minimal subset of RFC 8949: unsigned integers, negative integers,
// byte strings (definite length), text strings (definite length),
// arrays, maps, and false/true/null. WebAuthn never uses tags or
// floats in the structures we care about, so they're not implemented.
//
