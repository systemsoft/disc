...
Deno.test("assignRole — rejects unknown role", async () => {
  ...
  try {
    const auth = await provider.register({ email: "u@example.com", password: "password123" });
    const err = await assertRejects(() => provider.assignRole(auth.user.id, "ghost"), AuthError);
...
Deno.test("assignRole — rejects unknown user", async () => {
  ...
  try {
    await provider.createRole("admin");
    const err = await assertRejects(() => provider.assignRole("00000000-0000-0000-0000-000000000000", "admin"), AuthError);
...
Deno.test("login — JWT carries the user’s roles", async () => {
  ...
  try {
    ...
    const login = requireAuthResponse(await provider.login({ email: "u@example.com", password: "password123" }));
...
Deno.test("token snapshot — roles assigned after login do not affect existing token", async () => {
  ...
  try {
    ...
    /*** The old token must NOT carry the new role — that’s the documented snapshot semantics. ***/
    if (payload.roles !== undefined)
      assertEquals(payload.roles.includes("admin"), false);

    /*** But a fresh login picks it up. ***/
    const refreshed = requireAuthResponse(await provider.login({ email: "u@example.com", password: "password123" }));
