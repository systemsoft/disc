...
Deno.test("generateRecoveryCodes — regenerating invalidates the previous batch", async () => {
  ...
  try {
    ...
    for (const code of first) {
      assertEquals(await provider.consumeRecoveryCode(auth.user.id, code), false);
    }
...
Deno.test("generateRecoveryCodes — rejects unknown user", async () => {
  ...
  try {
    const err = await assertRejects(() => provider.generateRecoveryCodes("00000000-0000-0000-0000-000000000000"), AuthError);
...
Deno.test("generateRecoveryCodes — rejects out-of-range count", async () => {
  ...
  try {
    ...
    await assertRejects(() => provider.generateRecoveryCodes(auth.user.id, 0), AuthError);
    await assertRejects(() => provider.generateRecoveryCodes(auth.user.id, 100), AuthError);
  } finally {
    await db.close();
  }
});
...
Deno.test("consumeRecoveryCode — rejects code from a different user", async () => {
  ...
  try {
    const a = await provider.register({ email: "a@example.com", password: "password123" });
    const b = await provider.register({ email: "b@example.com", password: "password123" });
    const [aCode] = await provider.generateRecoveryCodes(a.user.id, 1);

    /*** User B trying to use A’s code — must not work even though the underlying hash exists. ***/
...
Deno.test("consumeRecoveryCode — rejects garbage", async () => {
  ...
  try {
    const auth = await provider.register({ email: "u@example.com", password: "password123" });
    assertEquals(await provider.consumeRecoveryCode(auth.user.id, "AAAAA-AAAAA"), false);
    assertEquals(await provider.consumeRecoveryCode(auth.user.id, "definitely not a code"), false);
  } finally {
    await db.close();
  }
});
...
Deno.test("loginWithRecoveryCode — completes login when TOTP is enrolled", async () => {
  ...
  try {
    ...
    const auth = await provider.loginWithRecoveryCode(challenge.challengeToken, codes[0]);
    assert(auth.token);
...
Deno.test("loginWithRecoveryCode — wrong code rejects, challenge stays valid for retry", async () => {
  ...
  try {
    ...
    const err = await assertRejects(() => provider.loginWithRecoveryCode(challenge.challengeToken, "AAAAA-AAAAA"), AuthError);
...
Deno.test("loginWithRecoveryCode — single-use challenge: cannot replay after success", async () => {
  ...
  try {
    ...
    const err = await assertRejects(() => provider.loginWithRecoveryCode(challenge.challengeToken, codes[1]), AuthError);
...
Deno.test("loginWithRecoveryCode — rejects unknown challenge token", async () => {
  ...
  try {
    const { userId } = await setupTOTP(provider);
    const codes = await provider.generateRecoveryCodes(userId, 1);
    const err = await assertRejects(() => provider.loginWithRecoveryCode("ghost-challenge-xxx", codes[0]), AuthError);
    assertEquals((err as AuthError).code, AuthErrorCode.INVALID_TOKEN);
  } finally {
    await db.close();
  }
...
async function setupTOTP(provider: AuthProvider): Promise<{ secret: string; userId: string; }> {
  const auth = await provider.register({ email: "u@example.com", password: "password123" });
  ...
  return { secret: enrollment.secret, userId: auth.user.id };
}
