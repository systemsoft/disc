...
Deno.test("RemoteCaptchaVerifier — happy path POSTs form-encoded body and parses success", async () => {
  ...
  const v = new RemoteCaptchaVerifier({ provider: "hcaptcha", secret: "shh" }, { fetchImpl });
...
Deno.test("RemoteCaptchaVerifier — turnstile uses Cloudflare verify URL by default", async () => {
  const { calls, fetchImpl } = makeFetch(() => new Response(JSON.stringify({ success: true }), { status: 200 }));
  const v = new RemoteCaptchaVerifier({ provider: "turnstile", secret: "shh" }, { fetchImpl });

  await v.verify("token");
  assertEquals(calls[0].url, "https://challenges.cloudflare.com/turnstile/v0/siteverify");
});
...
Deno.test("RemoteCaptchaVerifier — non-2xx response returns failure", async () => {
  ...
  const v = new RemoteCaptchaVerifier({ provider: "hcaptcha", secret: "x" }, { fetchImpl });
...
Deno.test("RemoteCaptchaVerifier — timeout aborts and returns network-error", async () => {
  ...
  const v = new RemoteCaptchaVerifier({ provider: "hcaptcha", secret: "x", timeoutMs: 10 }, { fetchImpl });
  const result = await v.verify("token");
