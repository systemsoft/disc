...
// Allow ±1 step on verification. RFC 6238 §6 calls this out: clock drift
// between client and server is real, and refusing to ever look at the
// previous/next window is hostile UX.
...
export async function verifyTOTP(
  ...
  // Walk the window comparing in constant time per slot. We *do* return
  // early after the first match — leaking which slot matched is not a
  // meaningful side channel (an attacker submitting a guess already knows
  // when they sent it).
...
async function hotp(
  ...
  // JS bitwise ops are 32-bit; split into hi/lo halves so we don't
  // truncate counters past 2^32 (unlikely with 30s steps, but cheap).
