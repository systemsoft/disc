...
// ── CBOR encoder (same subset our decoder handles) ───────────────────
...
// ── authenticatorData + attestationObject builders ───────────────────
...
function derInteger(bytes: Uint8Array): Uint8Array {
  // DER integers are signed; if the high bit of the first byte is set,
  // prepend 0x00 so it's not interpreted as negative.
  let val = bytes;
