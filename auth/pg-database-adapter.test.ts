...
class FakeDatabaseConnection {
  ...
  queryResult: QueryResult = { rowCount: 0, rows: [] };
  ...
  connect(): Promise<void> {
    this.calls.push({ args: [], method: "connect" });
...
Deno.test("convertPlaceholders - single placeholder", () => {
  assertEquals(convertPlaceholders("SELECT * FROM users WHERE id = ?"), "SELECT * FROM users WHERE id = $1");
});
...
Deno.test("PgDatabaseAdapter - execute converts placeholders", async () => {
  ...
  await adapter.execute("UPDATE users SET name = ? WHERE id = ?", ["Ada", "123"]);
