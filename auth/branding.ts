 ...
 */
export function buildMagicLinkUrl(token: string, opts: { baseUrl: string; template?: string; }): string {
  const encoded = encodeURIComponent(token);

  if (opts.template)
    return opts.template.replace(TOKEN_PLACEHOLDER, encoded);

  const trimmed = opts.baseUrl.endsWith("/") ?
...
function validateBrandColor(v: unknown): void {
  if (typeof v !== "string")
    throw new Error(`AuthProvider: branding.brandColor must be a string; got ${JSON.stringify(v)}`);
  ...
  /*** L: percent (0–100) or number (0–1). The regex already accepted the shape; here we
       range-check the magnitude. ***/
  const lRaw = oklchMatch[1];
  const lValue = parseFloat(lRaw);

  if (lRaw.endsWith("%")) {
    if (!(lValue >= 0 && lValue <= 100))
      throw new Error(`AuthProvider: branding.brandColor lightness ${JSON.stringify(lRaw)} is out of range (must be 0%–100%)`);
  } else {
    if (!(lValue >= 0 && lValue <= 1))
      throw new Error(`AuthProvider: branding.brandColor lightness ${JSON.stringify(lRaw)} is out of range (must be 0–1 when unitless, or use a percentage)`);
  }
  ...
  if (!(cValue >= 0 && cValue <= MAX_OKLCH_CHROMA))
    throw new Error(`AuthProvider: branding.brandColor chroma ${JSON.stringify(oklchMatch[2])} is out of range (must be 0–${MAX_OKLCH_CHROMA})`);

  /*** H: 0–360. CSS would mod-360 values >360, but rejecting surfaces typos faster. ***/
  const hValue = parseFloat(oklchMatch[3]);

  if (!(hValue >= 0 && hValue <= 360))
    throw new Error(`AuthProvider: branding.brandColor hue ${JSON.stringify(oklchMatch[3])} is out of range (must be 0–360)`);
  ...
  if (aRaw !== undefined) {
    ...
    if (aRaw.endsWith("%")) {
      if (!(aValue >= 0 && aValue <= 100))
        throw new Error(`AuthProvider: branding.brandColor alpha ${JSON.stringify(aRaw)} is out of range (must be 0%–100%)`);
    } else {
      if (!(aValue >= 0 && aValue <= 1))
        throw new Error(`AuthProvider: branding.brandColor alpha ${JSON.stringify(aRaw)} is out of range (must be 0–1 when unitless, or use a percentage)`);
    }
  }
}
...
export function validateBranding(branding: AuthBrandingConfig | undefined): void {
  ...
  if (branding.appName !== undefined) {
    ...
    if (v.length === 0)
      throw new Error("AuthProvider: branding.appName must be a non-empty string when set");
    ...
    if (hasControlChars(v))
      throw new Error("AuthProvider: branding.appName contains control characters (CR/LF/NUL) — these splice email headers and are not safe to render");
  }
  ...
  for (const key of ["logoUrl", "darkLogoUrl"] as const) {
    ...
    if (v.length === 0)
      throw new Error(`AuthProvider: branding.${key} must be a non-empty string when set`);

    if (v.length > MAX_URL_LEN)
      throw new Error(`AuthProvider: branding.${key} exceeds ${MAX_URL_LEN} chars`);

    if (hasControlChars(v))
      throw new Error(`AuthProvider: branding.${key} contains control characters — would break HTML rendering`);

    if (!isSafeUrlScheme(v))
      throw new Error(`AuthProvider: branding.${key} must be https:// (http:// is allowed only for localhost) — refuses data:/javascript:/file: schemes`);
  }
...
export function validateMagicLinkUrlTemplate(template: string | undefined): void {
  ...
  if (typeof template !== "string" || template.length === 0)
    throw new Error("AuthProvider: magicLinkUrlTemplate must be a non-empty string when set");

  if (template.length > MAX_URL_LEN)
    throw new Error(`AuthProvider: magicLinkUrlTemplate exceeds ${MAX_URL_LEN} chars`);

  if (hasControlChars(template))
    throw new Error("AuthProvider: magicLinkUrlTemplate contains control characters (CR/LF/NUL)");

  /*** `{token}` must appear exactly once. A URL ctor parse with the placeholder substituted to a
       stub also catches obviously broken templates (missing scheme, no host, etc.). ***/
  const occurrences = template.split(TOKEN_PLACEHOLDER).length - 1;

  if (occurrences !== 1)
    throw new Error(`AuthProvider: magicLinkUrlTemplate must contain exactly one "${TOKEN_PLACEHOLDER}" placeholder (got ${occurrences})`);

  const probe = template.replace(TOKEN_PLACEHOLDER, "TOKEN");

  if (!isSafeUrlScheme(probe))
    throw new Error("AuthProvider: magicLinkUrlTemplate must be https:// (http:// is allowed only for localhost)");
}
...
function hasControlChars(value: string): boolean {
  for (const ch of value) {
    ...
    /*** Strip ASCII C0 controls except TAB (\t = 9). DEL (127) and the C1 controls (128–159) are
         also banned: rendered HTML clients can interpret them ambiguously. ***/
