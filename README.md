...
> The `test`, `check`, `build`, `dev`, `serve`, `cli`, and `bench` tasks all chain `deno task prep` first. `prep` writes an empty `postgres/embedded-pg-manifest.ts` stub if missing (the real manifest is git-ignored and regenerated by `disc build`). When invoking `deno test`/`deno check` directly, run `deno task prep` once after cloning.
