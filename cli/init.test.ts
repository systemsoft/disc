...
Deno.test("CLI Init - basic project initialization", async () => {
  ...
  try {
    ...
    const schemaContent = await Deno.readTextFile(`${projectDir}/dbschema/default.disc`);
    ...
    /*** Verify .env content — managed mode does NOT pin DATABASE_URL (the DSN is derived from
         disc.toml at runtime). ***/
    const envContent = await Deno.readTextFile(`${projectDir}/.env`);
    assertStringIncludes(envContent, "DISC_PORT=5656");
    assertEquals(/^DATABASE_URL=/m.test(envContent), false, "managed-mode .env must not hardcode DATABASE_URL");
  } finally {
...
Deno.test("CLI Init - minimal template", async () => {
  ...
  try {
    Deno.chdir(tempDir);
    const projectDir = await initProject(tempDir, "minimal-project", { template: "minimal" });
    const schemaContent = await Deno.readTextFile(`${projectDir}/dbschema/default.disc`);
...
Deno.test("CLI Init - full template", async () => {
  ...
  try {
    Deno.chdir(tempDir);
    const projectDir = await initProject(tempDir, "full-project", { template: "full" });
    const schemaContent = await Deno.readTextFile(`${projectDir}/dbschema/default.disc`);
...
Deno.test("CLI Init - external --backend-dsn pins DATABASE_URL", async () => {
  /*** Replaces the legacy "custom database URL" mock test, which asserted a managed-mode
       DATABASE_URL pin that the modern scaffold no longer writes. The remaining behavior worth
       testing is the backend-DSN path: when the user opts into an external Postgres, the scaffold
  ...
  try {
    ...
    const projectDir = await initProject(tempDir, "custom-db-project", { backendDsn: customDbUrl });
    const envContent = await Deno.readTextFile(`${projectDir}/.env`);
...
Deno.test("CLI Init - directory already exists error", async () => {
  ...
  try {
    ...
    assertStringIncludes((thrown as Error).message, "already exists");
  } finally {
    Deno.chdir(originalCwd);
    await cleanupTempDir(tempDir);
  }
...
Deno.test("CLI Init - rejects invalid project name", async () => {
  /*** Replaces the prior pure-regex check with an end-to-end run: the real InitCommand throws when
       the name fails its validation. ***/
  const originalCwd = Deno.cwd();
  const tempDir = await createTempDir();
  ...
  try {
    ...
    for (const badName of ["My-Project", "disc_app", "-invalid", "invalid-"]) {
      ...
      assert(thrown instanceof Error, `name "${badName}" should be rejected by InitCommand`);
      assertStringIncludes((thrown as Error).message, "Invalid project name");
    }

    /*** A name with spaces hits Deno.mkdir / path semantics before reaching the regex; covered
         separately in the bad-name list above. Valid names round-trip without throwing —
         verify one. ***/
    await new InitCommand().execute({
      directory: tempDir,
      name: "blog-api",
...
Deno.test("CLI Init - creates proper README content", async () => {
  ...
  try {
    ...
    /*** The modern scaffold drives users to `disc start` (bundled-PG flow) rather than `createdb` —
         see `readme-test` below for the negative assertion. Here we exercise the positive
         content surface. ***/
...
Deno.test("CLI Init - throws on PG setup failure, keeps scaffold resumable", async () => {
  ...
  try {
    ...
    /*** Inject a PostgresManager that always fails — exercising the path that previously printed
         "✅ Project initialized successfully!" with exit 0. ***/
    ...
    assert(thrown instanceof Error, "execute() must throw when PG setup fails (fixes false-success exit 0 bug)");
    assertStringIncludes((thrown as Error).message, "simulated PG failure");

    /*** The scaffold should still exist so the user can re-run `disc start` after fixing
         the environment. ***/
    const tomlExists = await Deno
      .stat(`${tempDir}/${projectName}/disc.toml`)
      .then(() => true)
      .catch(() => false);
    assert(
      tomlExists,
...
Deno.test("CLI Init - disc.toml written inside project dir, not CWD", async () => {
  ...
  try {
    ...
    assert(!inCwd, "disc.toml must NOT leak to CWD (would be shared by sibling projects)");
  } finally {
    Deno.chdir(originalCwd);
    await cleanupTempDir(tempDir);
  }
...
Deno.test("CLI Init - disc.toml has correct managed instance config", async () => {
  ...
  try {
    ...
    const tomlContent = await Deno.readTextFile(`${tempDir}/${projectName}/disc.toml`);
...
Deno.test("CLI Init - deno.json scaffold omits unpublished JSR ref", async () => {
  ...
  try {
    ...
    const denoJson = JSON.parse(await Deno.readTextFile(`${tempDir}/${projectName}/deno.json`));
    /*** Until @disc/db is published on JSR, the scaffold must not reference it — a generated
         project that cannot resolve its imports is worse than a scaffold with no
         default imports. ***/
    assertEquals(denoJson.imports?.["@disc/db"], undefined, "Scaffold must not reference unpublished @disc/db via jsr:");
  } finally {
    Deno.chdir(originalCwd);
    await cleanupTempDir(tempDir);
  }
...
Deno.test("CLI Init - README does not reference manual createdb", async () => {
  ...
  try {
    ...
    const readme = await Deno.readTextFile(`${tempDir}/${projectName}/README.md`);
    /*** Scaffold used to tell users to `createdb foo_dev` — contradicts the bundled-PG
         value proposition. ***/
    assertEquals(readme.includes("createdb"), false, "README must not instruct manual createdb in the bundled-PG flow");
...
Deno.test("CLI Init - .env for managed PG omits TCP DATABASE_URL", async () => {
  ...
  try {
    ...
    /*** Previous scaffold wrote DATABASE_URL=postgresql://localhost:5432/disc_dev which
         contradicted the socket-only bundled PG. For managed mode the DSN is derived from
         disc.toml, so .env should not pin a specific URL. ***/
    assertEquals(/^DATABASE_URL=/m.test(env), false, ".env must not hardcode DATABASE_URL in managed mode");
  } finally {
    Deno.chdir(originalCwd);
    await cleanupTempDir(tempDir);
  }
...
Deno.test("CLI Init - schema at canonical dbschema/default.disc path", async () => {
  ...
  try {
    ...
    assert(schemaExists, "Schema should be at dbschema/default.disc (canonical path matching migrate/codegen defaults)");
    ...
    assert(!legacyExists, "Legacy schema.disc path should not be created");
 ...
 */
async function initProject(tempDir: string, name: string, extra: Partial<Parameters<InitCommand["execute"]>[0]> = {}): Promise<string> {
  await new InitCommand().execute({
    directory: tempDir,
    name,
    skipPostgres: true,
