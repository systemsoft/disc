...
Deno.test({
  fn: async () => {
    ...
    /*** Verify the user exists, the superuser role exists, and the assignment row in user_roles
         links them. ***/
    const db = new DatabaseConnection(dsn);
    await db.connect();

    try {
      const u = await db.query("SELECT id, email FROM users WHERE email = $1", ["root@example.com"]);
      assertEquals(u.rows.length, 1);
      const userId = u.rows[0].id as string;

      const r = await db.query("SELECT name FROM roles WHERE name = $1", ["superuser"]);
      assertEquals(r.rows.length, 1);

      const ur = await db.query(`SELECT role_name AS name FROM user_roles WHERE user_id = $1`, [userId]);
...
Deno.test({
  fn: async () => {
    ...
    try {
      const r = await db.query("SELECT 1 FROM users WHERE email = $1", ["u@example.com"]);
  ...
  fn: async () => {
    ...
    try {
...

      {
        ...
        try {
          const r = await db.query("SELECT password_hash FROM users WHERE email = $1", ["rotate@example.com"]);
      ...
      try {
        ...
        assert(newHash !== oldHash, "password_hash unchanged after admin set-password");
      } finally {
        await db.close();
      }
    } finally {
...
/*** gh/geldata#6432 — `disc admin list-policies` is pure SDL introspection, no DB required. Tests
     target the underlying `collectPoliciesFromSdl` helper so we exercise the policy-name + action +
     condition shape without a CLI runner. ***/
Deno.test("admin list-policies — collects policies from SDL with action + events + condition", () => {
  ...
  assertEquals(policies.has("Document"), true, "Document type should appear in policy list (Gel #6432)");
  assertEquals(policies.has("User"), false, "User has no policies — must not appear in the listing");
  ...
  assert(typeof ownerRead.condition === "string" && ownerRead.condition.length > 0, "owner_read should carry a condition string");

  const ownerWrite = docPolicies.find(p => p.name === "owner_write");
  assert(ownerWrite !== undefined);
  assertEquals(ownerWrite.events.sort(), ["delete", "update"], "owner_write should list both update + delete events");

  const adminOverride = docPolicies.find(p => p.name === "admin_override");
  assert(adminOverride !== undefined);
  assertEquals(adminOverride.errmessage, "Only admins can bypass document policies", "errmessage on admin_override must round-trip from SDL");
});
...
Deno.test("admin test-policy — collectAccessPolicyAst returns the AST nodes for the type (Bundle VV)", () => {
  ...
  assertEquals(map.has("Doc"), true, "Doc must surface in the AST policy map");

  const docPolicies = map.get("Doc")!;
  assertEquals(docPolicies.length, 2, "Doc declares 2 access policies");
  /*** Pinning AST shape — kind + name access path so a parser refactor that breaks
       `policy.name.value` trips here. ***/
...
Deno.test("admin test-policy — single target evaluates only the named policy", async () => {
  await withTempSdl(TEST_POLICY_SDL, async path => {
    ...
    assert(header.startsWith("Doc.owner_only (select):"), `Expected header to start with the qualified policy name; got: ${header}`);

    /*** No other policy headers should appear — only owner_only. ***/
    const policyHeaders = lines.filter(l => /^Doc\.\w+ \(select\):/.test(l));
    assertEquals(policyHeaders.length, 1, "Single-target mode must evaluate exactly one policy");
  });
});
...
Deno.test("admin test-policy — --all mode evaluates every policy on the type", async () => {
  await withTempSdl(TEST_POLICY_SDL, async path => {
    ...
    const policyHeaders = lines.filter(l => /^Doc\.\w+ \(select\):/.test(l));
    assertEquals(policyHeaders.length, 2, "--all mode must evaluate both policies on Doc");
    /*** Each header should report ALLOW or DENY. ***/
    for (const h of policyHeaders) {
      assert(/\b(ALLOW|DENY)\b/.test(h), `Each verdict line must carry ALLOW or DENY; got: ${h}`);
    }
  });
});
...
Deno.test("admin test-policy — denial surfaces the policy’s errmessage", async () => {
  await withTempSdl(TEST_POLICY_SDL, async path => {
    ...
    assert(/\bDENY\b/.test(joined), `Expected DENY verdict in: ${joined}`);
    /*** The `reason:` line should always be present. ***/
    assert(/\breason:/.test(joined), `Expected reason line in output: ${joined}`);
  });
});
...
Deno.test("admin test-policy — bad target shape throws clearly", async () => {
  await withTempSdl(TEST_POLICY_SDL, async path => {
    ...
    try {
      ...
      threw = true;
      assertStringIncludes((e as Error).message, "<Type>.<policy>");
    }

    assertEquals(threw, true, "Missing dot in target must throw with the expected hint");
  });
});
...
async function freshAuth(dsn: string): Promise<void> {
  /*** Tables may not exist before the first adminCommand.* call. Truncate each one independently so
       a missing table doesn’t abort the rest. ***/
  const db = new DatabaseConnection(dsn);
  await db.connect();
...
/*** gh/geldata#6432 slice 4 — `disc admin test-policy`. Run a policy in isolation against a
     synthetic context. Tests use a temp SDL file + the exported `testPolicyImpl` helper so we
     capture every emitted line without spinning up a subprocess. ***/
async function withTempSdl<T>(sdl: string, body: (path: string) => Promise<T>): Promise<T> {
