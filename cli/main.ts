  ...
  try {
    switch (command) {
      case "admin": {
        ...
        const baseOpts = { "database-url": args["database-url"], "jwt-secret": args["jwt-secret"] };
        ...
        switch (adminSub) {
          ...
          case "list-policies": {
            /*** gh/geldata#6432 — pure SDL introspection, no auth/DB hookup needed (operates on the
          ...
          case "test-policy": {
            /*** gh/geldata#6432 slice 4 — run a single policy in isolation against a synthetic
                 context. Pure SDL introspection + in-memory evaluator; no DB hookup. ***/
            ...
            /*** Globals come in as repeated `--global key=value` flags or a single
                 `--globals "k1=v1,k2=v2"` shorthand. The parser collapses repeats into an array;
                 normalize both forms into a record. ***/
      ...
      case "db": {
        ...
        switch (dbSubcommand) {
          ...
          case "push": {
            /*** gh/geldata#3761 — Prisma-style push. Applies the current schema to the live DB
                 without recording a migration. The dev-loop iteration command: rapid schema-edit →
                 push → test, no migration files until you’re ready for one. ***/
      ...
      case "deploy": {
        ...
        await commands.deploy({ format: args.format, output: args.output });
