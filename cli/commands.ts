...
export class CLICommands {
  ...
  async codegen(args: CLIArgs): Promise<void> {
    ...
    try {
      ...
      /*** Materialize the embedded SDK alongside the generated client so the
           `import { ... } from "./sdk/mod.ts"` line in client.ts resolves out of the box.
           Idempotent: re-extracts only when the marker file is missing OR pinned to a different
           binary version. ***/
  ...
  async dbDump(name: string, args: CLIArgs): Promise<void> {
    ...
    if (fmtRaw !== "plain" && fmtRaw !== "custom")
      throw new Error(`Invalid --format "${fmtRaw}". Valid values: plain, custom.`);
  ...
  async dbPush(args: CLIArgs): Promise<void> {
    ...
    try {
      ...
      console.log(`✓ Schema pushed (${result.value.length} ${result.value.length === 1 ? "migration" : "migrations"} applied, no history recorded).`);
    } finally {
      await pool.close();
    }
  }
  ...
  async migrate(args: CLIArgs): Promise<void> {
    ...
    /*** Rollback and squash do not need a schema source. Other branches (apply / --create /
         --status) do — load it once up front so multi-file projects don’t require `--schema` to
         point at every module file. Default is multi-file discovery in `./dbschema`. ***/
    ...
    /*** gh/geldata#7490: wire a progress listener into the SchemaManager so migrate runs surface
         per-step progress on stdout. Suppressed via `--quiet` and never emitted in dry-run (the
         dry-run path doesn’t execute migrations anyway). ***/
    const quiet = args.quiet === true;

    const onProgress = (!quiet && !dryRun) ?
   ...
   * (gh/geldata#702, #7469)
   */
  async schemaExport(args: { output?: string; schema?: string; "schema-dir"?: string; }): Promise<void> {
  ...
  async serve(options: ServeOptions): Promise<void> {
    ...
    try {
      ...
      /*** Load the project schema. Default is multi-file discovery in `./dbschema` (every `*.disc`
           file), so cross-module references resolve. `--schema <file>` opts back into
           single-file mode. ***/
      ...
      if (load) {
        ...
        /*** Auto-migrate on dev (managed PG only). External DSN is treated as user-managed;
             auto-applying DDL there could surprise an operator, so we only do it for the bundled
             instance. The migrate engine is a no-op if the live schema already matches
             the SDL. ***/
        if (ctx?.managed)
  ...
  async start(args: CLIArgs): Promise<void> {
    const ctx = resolveProjectContext();
    const projectName = ctx?.projectName || Deno.cwd().split("/").pop() ||
      "default";
    /*** Default behavior matches `systemctl start` — PG is a pg_ctl-managed daemon, so `disc start`
         returns as soon as PG is up. The health monitor (which used to block the foreground via a
  ...
  async status(_args: CLIArgs): Promise<void> {
    const ctx = resolveProjectContext();
    const projectName = ctx?.instanceName || Deno.cwd().split("/").pop() ||
      "default";
    ...
    try {
      ...
      console.log(`${statusIcon} Status: ${status.running ? "Running" : "Stopped"}`);

      if (status.running) {
        console.log(`   PID: ${status.pid}`);
        console.log(`   Port: ${status.port || "Unix socket only"}`);
        console.log(`   Started: ${status.startedAt?.toLocaleString() || "Unknown"}`);
        ...
        if (status.health) {
          ...
          console.log(`   Uptime: ${Math.floor((status.health.uptime || 0) / 60)} minutes`);
        }
      }
  ...
  private async applyMigrations(
    ...
    if (dryRun) {
      ...
      /*** Pre-flight: detect a running Disc server attached to the same database (gh/geldata#9034).
           ...
           schema cache won’t see the change until it reloads. We surface the warning *before*
           mutating the schema so the operator can plan a follow-up reload (the server’s
           schema-watch endpoint or a restart) rather than discover stale-cache failures after
           the fact. ***/
      try {
      ...
      /*** Live execution: applyModules handles diff + execute against the pre-merged Module[]
           (multi-file path). Pass through `allowUnsafe` so `--unsafe` callers aren’t blocked by the
           destructive-op gate (gh/geldata#1838).

           Per-migration progress is rendered by the progress listener wired in `migrate()`
           (gh/geldata#7490). The legacy "Migration Results" block was removed — the listener prints
  ...
  private async autoMigrateOnServe(modules: Module[]): Promise<void> {
    ...
    try {
      await pool.initialize();
      manager = new SchemaManager({ dryRun: false, pool });
      await manager.initialize();
  ...
  private async handleSquash(manager: SchemaManager, args: CLIArgs): Promise<void> {
    ...
    try {
      ...
      console.log(`  Combined rollback statements: ${result.rollbackStatements.length}`);
   ...
   */
  private async loadProjectSchema(args: { schema?: string; "schema-dir"?: string; }): Promise<ProjectSchemaLoad | null> {
    const schemaDir = args["schema-dir"] ?? "./dbschema";
    const schemaFile = args.schema;
    let sources: string[];
  ...
  private makeMigrateProgressListener(): MigrationProgressListener {
    ...
    return (event: MigrationProgressEvent): void => {
      switch (event.kind) {
        ...
        case "ddl-executing": {
          console.log(`    applying ${event.statementCount} DDL statement${event.statementCount === 1 ? "" : "s"} …`);
          return;
        }
        ...
        case "migration-failed": {
          console.error(`    ✗ failed: ${event.error} (after ${event.durationMs}ms${event.rollbackAttempted ? ", rollback attempted" : ""})`);
          return;
        }

        case "migration-started": {
          startedAt.set(event.migrationId, Date.now());
          console.log(`[${event.index}/${event.total}] ${event.name} …`);
        ...
        case "plan-completed": {
          ...
          console.log(`✓ Applied ${event.migrationCount} migration${event.migrationCount === 1 ? "" : "s"} in ${event.durationMs}ms`);
          return;
        }
   ...
   */
  private async readSchemaAsCompilerSchema(filePath: string): Promise<Schema | null> {
    ...
    try {
      ...
      console.error(`❌ Failed to convert schema: ${(error as Error).message}`);
      return null;
    }
  }
  ...
  private async showMigrationStatus(manager: SchemaManager, modules?: Module[]): Promise<void> {
    ...
    console.log(`  Current schema hash: ${status.currentSchemaHash || "(none)"}`);

    if (status.latestMigration) {
      console.log(`\n  Latest migration:`);
      console.log(`    ID: ${status.latestMigration.id}`);
      console.log(`    Name: ${status.latestMigration.name}`);
      console.log(`    Applied at: ${status.latestMigration.appliedAt.toISOString()}`);
    } else {
      console.log(`\n  No migrations have been applied yet.`);
    }

    /*** Drift detection (gh/geldata#8899). When the SDL has been loaded (single- or multi-file),
         diff it against the applied state so `migration --status` answers the question users
         actually ask: "is my schema in sync?". Renders one of three lines:
