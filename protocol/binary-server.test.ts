...
async function performNoAuthHandshake(
  ...
  // Read ParameterStatus messages + ReadyForCommand
  // We expect 2 ParameterStatus then 1 ReadyForCommand
  for (let i = 0; i < 3; i++) {
...
Deno.test("binary-server - no-auth handshake returns correct sequence", async () => {
  ...
  assertEquals(messages[5].kind, "ReadyForCommand");
  if (messages[5].kind === "ReadyForCommand") {
    assertEquals(
      messages[5].transactionState,
...
Deno.test("binary-server - auth handshake with SCRAM-SHA-256", async () => {
  ...
  // 10. Read ParameterStatus messages + ReadyForCommand
