...
import { BufferPool, MessagePool, CachedMessageBuilder } from "./pool.ts";
...
Deno.test("Benchmark - Message parsing", () => {
  ...
  const parseMessage: Types.ParseMessage = {
    ...
    commandText: "SELECT User { id, name, email, posts: { title, content } } FILTER .active = true ORDER BY .created_at DESC LIMIT 100",
  };
...
Deno.test("Benchmark - Message building", () => {
  ...
  const dataMessage: Types.DataMessage = {
    ...
    length: 0,
    dataElements: [
      { data: new TextEncoder().encode('{"id": 1, "name": "Alice", "email": "alice@example.com"}') },
      { data: new TextEncoder().encode('{"id": 2, "name": "Bob", "email": "bob@example.com"}') },
      { data: new TextEncoder().encode('{"id": 3, "name": "Charlie", "email": "charlie@example.com"}') },
    ],
  };
  ...
  const errorMessage: Types.ErrorResponse = {
    ...
    attributes: new Map([
      ...
      [Types.ErrorAttribute.Details, "The query parser encountered an unexpected token"],
    ]),
  };
...
Deno.test("Benchmark - End-to-end message roundtrip", () => {
  ...
  const executeMessage: Types.ExecuteMessage = {
    ...
    expectedCardinality: Types.Cardinality.Many,
    commandText: "SELECT User { id, name, email, profile: { bio, avatar_url }, posts: { id, title, content, tags, created_at } } FILTER .id = <uuid>$0",
    stateDataDescriptorId: Types.uuidToBytes("11111111-2222-3333-4444-555555555555"),
    encodedStateData: new Uint8Array(100).fill(42),
    argumentDataDescriptorId: Types.uuidToBytes("66666666-7777-8888-9999-aaaaaaaaaaaa"),
    argumentData: new Uint8Array(50).fill(7),
    outputDataDescriptorId: Types.uuidToBytes("bbbbbbbb-cccc-dddd-eeee-ffffffffffff"),
  };
