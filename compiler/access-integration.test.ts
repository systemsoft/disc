...
import { AccessPolicy, AccessContext } from "../access/mod.ts";
...
function createTestSchema(): Context.Schema {
  ...
  types.set("User", {
    ...
    properties: new Map([
      ...
      ["tenant_id", { name: "tenant_id", type: "int", required: true, multi: false, columnName: "tenant_id" }],
    ]),
    links: new Map(),
  });
  ...
  types.set("Document", {
    ...
    properties: new Map([
      ...
      ["content", { name: "content", type: "str", required: true, multi: false, columnName: "content" }],
      ["public", { name: "public", type: "bool", required: false, multi: false, columnName: "public" }],
      ["owner_id", { name: "owner_id", type: "uuid", required: true, multi: false, columnName: "owner_id" }],
      ["tenant_id", { name: "tenant_id", type: "int", required: true, multi: false, columnName: "tenant_id" }],
    ]),
    links: new Map(),
  });
...
Deno.test("Access Control - Deny without authentication", () => {
  ...
  const compiler = new EdgeQLCompilerWithAccess(
    schema,
    { mode: "restrictive", defaultAllow: false, enableRLS: true, enableAudit: false }
  );
