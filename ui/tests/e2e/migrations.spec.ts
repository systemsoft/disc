...
test.describe("Migrations page", () => {
  ...
  test("renders the auto-applied initial migration row", async ({ page }) => {
    await page.goto("/ui/migrations");
    /*** The fixture project’s first `disc serve` auto-applies a migration creating the Item type.
         Wait for the table to populate. ***/
    await expect(page.getByRole("cell", { name: "create_item" })).toBeVisible({ timeout: 10_000 });
    /*** Header row is present. ***/
    await expect(page.getByRole("columnheader", { name: "Applied" })).toBeVisible();
    await expect(page.getByRole("columnheader", { name: "Name" })).toBeVisible();
    await expect(page.getByRole("columnheader", { name: "Schema Hash" })).toBeVisible();
  });
