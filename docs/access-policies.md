...
The implementation lives in `server/http-handlers.ts:handle_query` (header parsing + role gate) and `compiler/compiler.ts:applyAccessControl` (short-circuit on `AccessContext.bypass`). (gh/geldata#6358)
