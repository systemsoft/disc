...
| UU     | #6432 slice 3: per-policy session disable via `X-Disc-Disable-Policies` header (admin-gated, cache-key-isolated). Slice 4 stays open.    | `51f98a8`                                  |
| VV     | #6432 slice 4: `disc admin test-policy <Type>.<policy>` (with `--all` mode) — run-in-isolation policy debugger. Closes #6432.            | (this bundle)                              |
...
>
> **Bundle RR closed the migration-robustness sub-cluster (#3761 + #5190 + #6697).**
> #3761 shipped `disc db push` (Prisma-style schema push, no migration history);
> #5190 + #6697 are structurally inapplicable (Disc has a single trunk with
...
>
> **Bundles SS + UU + VV closed the entire auth/access-introspection
> sub-cluster (#6432 + #8909).** Slice 1 (errmessage) was already in
> `access/evaluator.ts`; slice 2 (`disc admin list-policies`) shipped
> in SS; slice 3 (`X-Disc-Disable-Policies` header) shipped in UU;
> slice 4 (`disc admin test-policy` run-in-isolation debugger) shipped
> in VV; slice 5 (narrative docs) is incrementally covered in
> `docs/access-policies.md`. #8909 is tied to #6697 (no semver-major
> in-place upgrades in Disc) and is structurally inapplicable.
